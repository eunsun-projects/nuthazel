...
export default function List({data, setModal, setCurrNum, setList, type}){
    ...
    const deleteFetch = (item) => async () => {
        if (confirm('해당 항목을 삭제하시겠습니까?')){
            ...
            try{
                ...
                const setResponse = await fetch('/api/nutset', req);
            ...
            {data.map((e,i) => {
                return(
                    <div key={i} className={styles.toonlist}>
                        <div className={styles.toonlistthumb} style={{backgroundImage: `url(${e.imgurl[0]})`}}></div>
                        ...
                        <div className={styles.toonlisttext}>
                            ...
                            <div style={{display: "flex", flexDirection: "row", gap: "1rem"}}>
                                <div className={styles.admincrudbtn} onClick={handleClick(e.num)}>수정</div>
                                <div className={styles.admincrudbtn} onClick={deleteFetch(e)}>삭제</div>
                            </div>
                        </div>
