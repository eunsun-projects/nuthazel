...
export default function Modal({ setModalOpen, collabocontents, setQueryNum }){
    const router = useRouter();
    const pathname = usePathname();
    const searchParams = useSearchParams();

    // collabo 스테이트는 이미지 정방인지 뭔지 판단하는 것임
    const [collabo, setCollabo] = useState(null);
    ...
    useLayoutEffect(()=>{
        ...
        if(collabocontents !== null){
            ...
            Promise.all(copy.imgurl.map(e => sizePromises(e)))
                .then(result => {
                    copy.sizes = result;
                    // 완성된 카피 객체로 setState
                    setCollabo(copy);
                    ...
                    <div className={styles.modalcontents}>
                        <div className={styles.contents}>
                            ...
                            <div className={styles.modalcondesc}>
                                <p style={{paddingBottom: collabocontents.link.title.length > 0 ? "1rem" : "0px"}}>{collabocontents.desc}</p>
                                {/** length 로 판단하는 이유는, collabo 면 nutadmin에서 link 객체가 무조건 있기 때문에*/}
                                {collabocontents.link.title.length > 0 && (
                                    <Link href={collabocontents.link.href} target="_blank" style={{color:'#916b4d'}}>
                                ...
                                <div className={styles.modalconimgbox}>
                                    {!collabo && (
                                        <div style={{position: "relative", width: "100%", height: "100%", display: "flex", justifyContent: "center", alignItems: "center"}}>
                                            <img style={{position: "relative", width: "20%", height: "100%", objectFit: "contain"}} src="/assets/loading.gif"></img>
                                        </div>    
                                    )}

                                    {collabo &&
                                        collabo.imgurl.map((e, i)=>{
