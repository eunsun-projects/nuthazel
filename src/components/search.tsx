...
export default function Search({ searchDb }: SearchProps) {
  ...
  const [searchResult, setSearchResult] = useState<
    { keywords: string; title: string; pageurl: string }[]
  >([]);
  ...
  function searching(searchString: string) {
    if (searchString.length > 20) {
      alert("20자 이내로 적어주세요.");
    } else {
      const sanitized = sanitizeInput(searchString);
      let searchResultArr: string[] = []; //최종 검색결과를 담을 어레이
      let nonbspSearchArr: string[] = []; //공백이 제거된 검색어를 담을 어레이
      ...
      if (searchResultArr.length === 0) {
        ...
        let resultArr: { keywords: string; title: string; pageurl: string }[] =
          [];
        searchResultArr.forEach((e) => {
          const mapped = searchDb.map((item) => {
            const temp = item.keywords.filter((a: string) => a.includes(e));
  ...
  //검색모달 닫기
  const handleSearchModalClick = (e: React.MouseEvent<HTMLDivElement>) => {
